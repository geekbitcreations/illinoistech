...
function linkDocument(xmlDoc) {
   ...
   for (var i = 0; i < allElems.length; i++) {
      ...
      if (allElems[i].getAttribute("id") == "previewTitle") this.title =allElems[i].firstChild.nodeValue;
      if (allElems[i].getAttribute("id") == "previewImage") this.image = allElems[i].getAttribute("src");
      if (allElems[i].getAttribute("id") == "previewParagraph") this.paragraph = allElems[i].firstChild.nodeValue;

   }

}

linkDocument.prototype.makeOverlay = function() {

   var overlay = document.createElement("div");
   overlay.className = "linkOverlay";
   ...
   paraElem.innerHTML = this.paragraph;

   overlay.appendChild(titleElem);
   overlay.appendChild(imageElem);
   overlay.appendChild(paraElem);
