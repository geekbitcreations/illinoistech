/*
   ...
   rankings
      An array of elements belonging to the rdrop class

   portraits
      An array of elements belonging to the pres class
   ...
   diffX
      ...
      mousePortrait and the mouse pointer
   ...
   hoverRanking
      The ranking square that the moving portrait is currently hovered over
   ...
   init()
      Sets up and initializes the Web page, defining the ranking and portraits array,
      and appying event handlers to mouse and keyboard actions

   showDesc(e)
      Shows the text description of each president in the text area box, presText.

   removeDesc(e)
      Removes the text description of each president from the text area box, presText
      during a mouseout event occuring over the presidential portrait.

   dropValid(object)
      Returns a Boolean value indicating whether it valid to drop the object. The function
      is false if dropping object will cover a portrait and true if otherwise

   alignPortrait(object)
      If object is over a ranking square, aligns object with the top-left corner of the square

   highlightRanking(object)
      If object is over a ranking square, sets the background color of the square to light green

   mouseGrab(e)
      "Grabs" a portrait using the mousedown action. Sets the value of mousePortrait. Calculates
      the value of diffX and diffY. Applies event handlers for mousemove and mouseup events

   mouseMove(e)
      Move mousePortrait across the Web page, keeping a constant distance from the mouse pointer

   mouseDrop(e)
      Drops mousePortrait on the Web page (if it is not over another portrait). Aligns the
      portrait with the ranking. Turns off event handlers for the mousemove and mouseup events.
...
var mousePortrait = null;

var diffX = null;
var diffY = null;
var maxZ = 1;
var hoverRanking = null;
...
var presDesc = new Array();
presDesc[0] = "Roosevelt, Theodore (1901 - 09) \n\n";
presDesc[0] += "Known as the bull moose, Roosevelt was a man of ";
presDesc[0] += "extraordinary discipline ";
...
presDesc[2] += "In hist first term, Wilson enacted a program of ";
presDesc[2] += "domestic reform, including the Federal Reserve Act. Reelected in ";
presDesc[2] += "1916 as a peace candidate, he tried to mediate between the ";
presDesc[2] += "warring nations; but when the Germans resumed unrestricted submarine warfare in 1917, ";
presDesc[2] += "Wilson brought the United States into the war. After the conclusion of the war, he strove ";
presDesc[2] += "unsuccessfully to lay the foundations for enduring peace.";
...
presDesc[8] += "Eisenhower was elected ";
presDesc[8] += "president in 1952 over Adlai E. Stevenson. Through two terms, Eisenhower hewed to moderate domestic ";
presDesc[8] += "policies. He sought peace through Free World strength in an era of new nationalisms, nuclear missiles, ";
...
function init() {
   ...
   for (var i = 0; i < allElem.length; i++) {
      if (allElem[i].className == "rdrop") rankings.push(allElem[i]);
      if (allElem[i].className == "pres") portraits.push(allElem[i]);
   }

   for (var i = 0; i < portraits.length; i++) {
      portraits[i].style.backgroundImage = "url(pres" + i + ".jpg)";
      portraits[i].title = presTitle[i];
      portraits[i].style.top = getStyle(portraits[i],"top");
      portraits[i].style.left = getStyle(portraits[i],"left");
      portraits[i].style.width = getStyle(portraits[i],"width");
      portraits[i].style.height = getStyle(portraits[i],"height");
      portraits[i].style.cursor = "pointer";

      addEvent(portraits[i], "mousedown", mouseGrab, false);
      addEvent(portraits[i], "mouseover", showDesc, false);
      addEvent(portraits[i], "mouseout", removeDesc, false);
   }

   for (var i = 0; i < rankings.length; i++) {
      rankings[i].style.top = getStyle(rankings[i],"top");
      rankings[i].style.left= getStyle(rankings[i],"left");
      rankings[i].style.width = getStyle(rankings[i],"width");
      rankings[i].style.height = getStyle(rankings[i],"height");
   }
...
function dropValid(object) {
   ...
   for (var i = 0; i < portraits.length; i++) {
      if (withinIt(object, portraits[i])) return false;
   }

   return true;
}

function alignPortrait(object) {
   for (var i = 0; i < rankings.length; i++) {
      if (withinIt(object, rankings[i])) {
            object.style.left = rankings[i].style.left;
            object.style.top = rankings[i].style.top;
            break;
      }
   } 
}

function highlightRanking(object) {
   if (hoverRanking) hoverRanking.style.backgroundColor = "";   

   for (var i = 0; i < rankings.length; i++) {
      if (withinIt(object, rankings[i])) {
         hoverRanking= rankings[i];
         hoverRanking.style.backgroundColor = "rgb(245, 226, 167)";
         break;
      }
   }
}
...
function mouseGrab(e) {
   ...
   mousePortrait = evt.target || evt.srcElement;
   
   maxZ++;
   mousePortrait.style.zIndex = maxZ; // place the portrait above other objects
   mousePortrait.style.cursor = "move";

   var mouseX = evt.clientX; // x-coordinate of pointer
   var mouseY = evt.clientY; // y-coordinate of pointer

   /* Calculate the distance from the pointer to the portrait */
   diffX = parseInt(mousePortrait.style.left) - mouseX; 
   diffY = parseInt(mousePortrait.style.top) - mouseY;
...
function mouseMove(e) {
   ...
   mousePortrait.style.left = mouseX + diffX + "px";
   mousePortrait.style.top = mouseY + diffY + "px";
   highlightRanking(mousePortrait);   
}

function mouseDrop(e) {
   if (dropValid(mousePortrait)) {
      alignPortrait(mousePortrait);
      removeEvent(document, "mousemove", mouseMove, false);
      removeEvent(document, "mouseup", mouseDrop, false);
      mousePortrait.style.cursor = "pointer";
   }
}
