...
function sliderBox(input, first, last, inc, style) {
   this.input = input;
   this.first = first;
   this.last = last;
   this.inc = inc
   this.style = style;
   this.range = this.last - this.first;
   this.line = null;
   this.bar = null;

   this.divElem = document.createElement("div");
   this.divElem.className = "sliderBox";
}


function sliderLine() {
   this.divElem = document.createElement("div");
   this.divElem.className = "sliderLine";
}


function sliderBar(style) {
   this.divElem = document.createElement("div");
   this.divElem.className = "sliderBar" + style;
}


sliderBox.prototype.constructDOM = function() {
   this.line = new sliderLine();
   this.bar = new sliderBar(this.style);

   this.line.divElem.appendChild(this.bar.divElem);
   this.divElem.appendChild(this.line.divElem);
}

sliderBar.prototype.position = function() {
   return parseFloat(this.divElem.style.left);
}

sliderBar.prototype.setPosition = function(pos) {
   this.divElem.style.left = pos + "px";
}

sliderBox.prototype.getSizes = function() {
   this.lineWidth = parseFloat(getStyle(this.line.divElem, "width"));
   this.barWidth = parseFloat(getStyle(this.bar.divElem, "width"));
   this.barMaxTravel = this.lineWidth - this.barWidth;
   this.ratio = this.range/this.barMaxTravel;
   this.bar.setPosition(0);
}

sliderBox.prototype.updateValue = function() {

   this.input.focus();
   var pos = this.bar.position();
   this.input.value = this.inc*Math.round((this.first + pos*this.ratio)/this.inc);
   this.input.blur();
