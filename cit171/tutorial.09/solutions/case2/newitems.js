...
addEvent(window, "load", highlightNew, false);


function highlightNew() {

   // Retrieve the date of last visit
   if (getCookie("lastVisit") != null) lastVisit = new Date(getCookie("lastVisit"))
   else lastVisit = null;

   // Retrieve date of current visit
   var today = new Date("August 13, 2013 13:00:00");
   var currentVisit = writeDateTime(today);
   setCookie("lastVisit", currentVisit);

   if (lastVisit != null) {
      var lastDateElem = document.createElement("span");
      lastDateElem.id = "lastDate";
      lastDateElem.innerHTML = "You last visited on " + lastVisit;
      document.body.appendChild(lastDateElem);

      var allElems = document.getElementsByTagName("*");
      var allStories = new Array();
      for (var i = 0; i < allElems.length; i++) {
         if (allElems[i].className == "posttime") allStories.push(allElems[i]);
      }
      for (var i = 0; i < allStories.length; i++) {

         var storyDate = new Date(allStories[i].innerHTML.slice(8));
         if (lastVisit < storyDate) {
            newItemImage = document.createElement("img");
            newItemImage.src = "newitem.jpg";
            allStories[i].insertBefore(newItemImage, allStories[i].firstChild);
         }
      }
   }
